...
type ListingStatus = "DRAFT" | "PENDING" | "APPROVED" | "REJECTED" | "EXPIRED" | "HIDDEN";
    ...
    <div ref={ref} className="relative inline-block text-left">
      <div className="flex items-center gap-2 justify-end">
        ...
        <button
          ...
          disabled={isBusy}
          onClick={() => setOpen((v) => !v)}
          className="inline-flex items-center justify-center gap-1.5 rounded-lg bg-[var(--primary)] px-3 py-1.5 text-xs font-semibold text-white hover:bg-[var(--primary-hover)] disabled:opacity-50 transition-all shadow-sm shadow-[var(--primary)]/20"
        >
          {isBusy ? (
...
export default function TinDangPage() {
  ...
  // Sync state from URL on load/change
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    fetch("/api/moi-gioi/listings")
      .then((r) => r.json())
      .then((res) => {
        if (res.data) setListings(res.data);
        else if (res.error) setError(String(res.error));
      })
  ...
  return (
    <div className="page-container">
      ...
      {/* Main Content */}
      <div className="card-container min-h-[500px] flex flex-col">
        {/* Toolbar */}
        ...
        <div className="flex-1 overflow-x-auto">
          {loading ? (
            <div className="flex flex-col items-center justify-center p-20">
              ...
              <p className="mt-4 text-sm text-[var(--muted-foreground)]">Đang tải dữ liệu...</p>
            </div>
          ) : filtered.length === 0 ? (
