...
const ROLE_CONFIG: Record<UserRole, { label: string; className: string }> = {
    ...
    AGENT: { label: "Môi giới", className: "badge-primary" },
...
export function UsersAdminClient({
    ...
    const initialRole = (searchParams.get("role") as "ALL" | UserRole) || "ALL";

    const [keyword, setKeyword] = useState(initialKeyword);
    const [roleFilter, setRoleFilter] = useState<"ALL" | UserRole>(initialRole);
    ...
    useEffect(() => {
        ...
        }, 500); // debounce URL update

        return () => clearTimeout(timeout);
    }, [keyword, roleFilter, pathname, router, initialKeyword, initialRole]);
                    ...
                    <div className="flex h-[600px] w-full max-w-md flex-col overflow-hidden rounded-2xl bg-[var(--card)] shadow-2xl ring-1 ring-black/10">
                        ...
                        <div className="flex-1 overflow-y-auto bg-[var(--background)] p-4 space-y-4 scroll-smooth">
                            {loadingChat ? (
                                ...
                                messages.map((m) => {
                                    const isMyMsg = m.senderId === currentUserId;
                                    return (
                    ...
                    <div className="p-1">
                        ...
                        {Object.keys(ROLE_CONFIG).map((role) => (
                            role !== user.role && (
                                ...
                                >
                                    <span className={`h-2 w-2 rounded-full ${role === 'ADMIN' ? 'bg-purple-500' : role === 'AGENT' ? 'bg-blue-500' : 'bg-slate-500'}`}></span>
                        ...
                        <button
                            ...
                            onClick={() => {
                                setOpen(false);
                                onToggleLock(user.id, user.isLocked);
                            }}
