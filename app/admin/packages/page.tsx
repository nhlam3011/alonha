...
type PackageRow = {
  ...
  price: number; durationDays: number | null; isActive: boolean; sortOrder: number; usageCount: number;
};
type PackageForm = {
  id: string | null; code: string; name: string; description: string;
  price: string; durationDays: string; sortOrder: string; isActive: boolean;
};

const INITIAL: PackageForm = { id: null, code: "", name: "", description: "", price: "", durationDays: "", sortOrder: "0", isActive: true };
function toForm(i: PackageRow): PackageForm {
  return { id: i.id, code: i.code, name: i.name, description: i.description || "", price: String(i.price), durationDays: i.durationDays != null ? String(i.durationDays) : "", sortOrder: String(i.sortOrder), isActive: i.isActive };
}
...
export default function AdminPackagesPage() {
  ...
  async function load(signal?: AbortSignal) {
    ...
    try {
      const res = await fetch("/api/admin/packages", { signal });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error || "Lỗi tải gói.");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const payload = { ...(form.id ? { id: form.id } : {}), code: form.code || undefined, name: form.name, description: form.description || null, price: Number(form.price), durationDays: form.durationDays ? Number(form.durationDays) : null, sortOrder: Number(form.sortOrder || 0), isActive: form.isActive };
      const res = await fetch("/api/admin/packages", { method: form.id ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      const data = await res.json().catch(() => ({}));
  ...
  async function handleDelete(id: string) {
    ...
    try {
      const res = await fetch("/api/admin/packages", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) });
      const data = await res.json().catch(() => ({}));
  ...
  async function toggleActive(item: PackageRow) {
    setSaving(true); setError(null);
    try {
      const res = await fetch("/api/admin/packages", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: item.id, isActive: !item.isActive }) });
      const data = await res.json().catch(() => ({}));
