...
export default function LoanCalculatorPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-[var(--foreground)]">Tính lãi suất vay</h1>
        <p className="mt-2 text-sm sm:text-base text-[var(--muted-foreground)]">Ước tính lịch trả nợ chi tiết dựa trên số tiền, thời hạn và lãi suất.</p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-12">
        ...
        <div className="lg:col-span-4">
          <div className="card-container">
            <div className="card-header"><h2 className="text-sm font-bold text-[var(--foreground)]">Thông tin khoản vay</h2></div>
            <div className="card-body space-y-5">
              {/* Số tiền vay */}
              <div>
                <div className="mb-2 flex items-center justify-between">
                  <span className="text-xs font-semibold text-[var(--muted-foreground)] uppercase tracking-wider">Số tiền vay</span>
                  <span className="text-sm font-bold text-[var(--primary)]">{fmtBillion(principal)}</span>
                </div>
                ...
                <div className="mt-1 flex justify-between text-[10px] text-[var(--muted-foreground)]"><span>100tr</span><span>20 tỷ</span></div>
              </div>
              {/* Thời hạn */}
              <div>
                <div className="mb-2 flex items-center justify-between">
                  <span className="text-xs font-semibold text-[var(--muted-foreground)] uppercase tracking-wider">Thời hạn</span>
                  <span className="text-sm font-bold text-[var(--foreground)]">{years} năm</span>
                </div>
                <input type="range" min={1} max={35} step={1} value={years}
                  onChange={(e) => setYears(Number(e.target.value))}
                  className="w-full h-1.5 rounded-lg appearance-none cursor-pointer bg-[var(--muted)] accent-[var(--primary)]" />
                <div className="mt-1 flex justify-between text-[10px] text-[var(--muted-foreground)]"><span>1 năm</span><span>35 năm</span></div>
              </div>
              {/* Lãi suất */}
              <div>
                <div className="mb-2 flex items-center justify-between">
                  <span className="text-xs font-semibold text-[var(--muted-foreground)] uppercase tracking-wider">Lãi suất</span>
                  <span className="text-sm font-bold text-[var(--foreground)]">{rateYear}%/năm</span>
                </div>
                <input type="range" min={1} max={20} step={0.1} value={rateYear}
                  onChange={(e) => setRateYear(Number(e.target.value))}
                  className="w-full h-1.5 rounded-lg appearance-none cursor-pointer bg-[var(--muted)] accent-[var(--primary)]" />
                <div className="mt-1 flex justify-between text-[10px] text-[var(--muted-foreground)]"><span>1%</span><span>20%</span></div>
              </div>
              {/* Phương thức */}
              <div>
                <p className="mb-2 text-xs font-semibold text-[var(--muted-foreground)] uppercase tracking-wider">Phương thức</p>
                <div className="grid grid-cols-2 gap-1.5 rounded-lg bg-[var(--muted)] p-1">
                  <button type="button" onClick={() => setMethod("equal_installment")}
                    className={`rounded-md px-2 py-2 text-xs font-semibold transition-all ${method === "equal_installment" ? "bg-[var(--card)] text-[var(--primary)] shadow-sm" : "text-[var(--muted-foreground)]"}`}>
                    Dư nợ giảm dần
                  </button>
                  <button type="button" onClick={() => setMethod("equal_principal")}
                    className={`rounded-md px-2 py-2 text-xs font-semibold transition-all ${method === "equal_principal" ? "bg-[var(--card)] text-[var(--primary)] shadow-sm" : "text-[var(--muted-foreground)]"}`}>
                    Gốc đều
                  </button>
                </div>
              </div>
        ...
        <div className="lg:col-span-8 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="card-container bg-[var(--primary-light)]">
              <div className="card-body text-center py-4">
                <p className="text-xs font-medium text-[var(--primary)]">Trả tháng đầu</p>
                <p className="mt-1 text-xl sm:text-2xl font-extrabold text-[var(--primary)]">{fmtBillion(monthlyFirst)}</p>
              </div>
            </div>
            <div className="card-container">
              <div className="card-body text-center py-4">
                <p className="text-xs font-medium text-[var(--muted-foreground)]">Tổng lãi</p>
                <p className="mt-1 text-xl sm:text-2xl font-extrabold text-[var(--foreground)]">{fmtBillion(totalInterest)}</p>
              </div>
            </div>
            <div className="card-container">
              <div className="card-body text-center py-4">
                <p className="text-xs font-medium text-[var(--muted-foreground)]">Tổng gốc + lãi</p>
                <p className="mt-1 text-xl sm:text-2xl font-extrabold text-[var(--foreground)]">{fmtBillion(totalPayment)}</p>
              </div>
            </div>
          </div>
            ...
            <div className="card-body">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-5">
                ...
                <div className="flex-1 w-full space-y-2">
                  ...
                  <div>
                    <div className="mb-0.5 flex justify-between text-xs">
                      <span className="flex items-center gap-1.5 text-[var(--muted-foreground)]"><span className="h-2 w-2 rounded-full bg-[var(--destructive)]" />Lãi</span>
                      <span className="font-bold text-[var(--foreground)]">{fmtBillion(totalInterest)}</span>
                    </div>
                    <div className="h-1.5 rounded-full bg-[var(--muted)]"><div className="h-full rounded-full bg-[var(--destructive)] transition-all" style={{ width: `${interestPct}%` }} /></div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="card-container overflow-hidden">
            <button type="button" onClick={() => setShowSchedule(!showSchedule)}
              className="flex w-full items-center justify-between px-4 sm:px-5 py-3 text-left transition-colors hover:bg-[var(--muted)]/50">
                ...
                <div className="overflow-x-auto max-h-[400px]">
                  <table className="data-table w-full text-xs">
                    <thead>
                      <tr>
                        <th>Năm</th>
                        <th className="text-right text-[var(--primary)]">Gốc</th>
                        <th className="text-right text-[var(--destructive)]">Lãi</th>
                        <th className="text-right">Tổng</th>
                        <th className="hidden sm:table-cell text-right">Dư nợ</th>
                      </tr>
                    </thead>
                    <tbody>
                      {yearlyRows.map((r) => (
                        <tr key={r.year}>
                          <td className="font-medium">Năm {r.year}</td>
                          <td className="text-right text-[var(--primary)]">{fmtBillion(r.principal)}</td>
                          <td className="text-right text-[var(--destructive)]">{fmtBillion(r.interest)}</td>
                          <td className="text-right font-bold">{fmtBillion(r.payment)}</td>
                          <td className="hidden sm:table-cell text-right text-[var(--muted-foreground)]">{fmtBillion(r.endBalance)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
